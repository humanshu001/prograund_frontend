  ...
  return (
    <>
      {loading ? (
        ...
        <>
          {posts.map((item) => (
            <div key={item.post_id}><PostItem title={item.title} post_id={item.post_id} time={item.uploaded_time} likes_count={item.likes_count} user_id={parseInt(item.user_id)} image_link={item.file} username={item.user? item.user.username:"ProGraund User"} image={item.user?item.user.image:null} share_count={item.share_count}/></div>
          ))}
